    ...
    nodeTypes: {
      project: {
        valueSchema: { type: "object", properties: {
          capacity: { $ref: "#/$defs/quantity" }, note: { type: ["string", "null"] },
          "a/b~c": { type: "string" }
        }, $defs: { quantity: { type: "number", minimum: 0 } }, additionalProperties: false },
...
const initial: GraphOp[] = [
  { op: "node.add", id: "c1", nodeType: "project" },
  { op: "node.add", id: "c2", nodeType: "project" },
  { op: "node.add", id: "document", nodeType: "document" },
  { op: "set", nodeId: "c1", path: "/capacity", value: 10 },
  { op: "edge.add", id: "e1", relationType: "uses", from: "c1", to: "document" },
  { op: "edge.add", id: "e2", relationType: "uses", from: "c2", to: "document" }
];
...
test("one batch fills a graph with a shared reference and increments version once", () => {
  ...
  assert.deepEqual(result.nodes.c1?.fields.capacity, { kind: "value", value: 10 });
...
test("preview and edit produce identical candidates; previews consume no state or IDs", () => {
  ...
  preview.candidate.nodes.c1!.fields.capacity = { kind: "value", value: 999 };
  preview.candidate.tombstones.nodes.push("corruption");
  assert.deepEqual(engine.getDraft(draft.id).nodes.c1?.fields.capacity, { kind: "value", value: 10 });
...
test("invalid final field values and relations roll back graph, version and tombstones", () => {
  ...
  for (const tail of [
    { op: "set", nodeId: "c2", path: "/capacity", value: -1 },
    { op: "edge.add", id: "bad", relationType: "uses", from: "document", to: "c2" },
    { op: "edge.add", id: "bad", relationType: "missing", from: "c2", to: "document" },
...
test("deleting a shared target requires explicit removal of every incoming edge", () => {
  const { engine, draft } = setup();
  engine.edit(draft.id, 0, initial);
  assert.throws(() => engine.edit(draft.id, 1, [{ op: "node.remove", id: "document" }, { op: "edge.remove", id: "e1" }]), hasCode("INVALID_GRAPH"));
  const result = engine.edit(draft.id, 1, [{ op: "node.remove", id: "document" }, { op: "edge.remove", id: "e1" }, { op: "edge.remove", id: "e2" }]);
  assert.deepEqual(result.edges, {});
  assert.equal(Object.hasOwn(result.nodes, "document"), false);
...
test("deleted node and edge identities cannot be reused within a batch or later", () => {
  const { engine, draft } = setup();
  engine.edit(draft.id, 0, initial);
  assert.throws(() => engine.edit(draft.id, 1, [{ op: "node.remove", id: "c1" }, { op: "node.add", id: "c1", nodeType: "document" }]), hasCode("ID_ALREADY_USED"));
...
test("failed batches do not reserve newly introduced IDs", () => {
  const { engine, draft } = setup();
  assert.throws(() => engine.edit(draft.id, 0, [...initial, { op: "set", nodeId: "c1", path: "/capacity", value: "bad" }]), hasCode("INVALID_GRAPH"));
  assert.deepEqual(engine.getDraft(draft.id), draft);
...
test("operations run in order while structural validity is checked on the final graph", () => {
  const { engine, draft } = setup();
  assert.throws(() => engine.edit(draft.id, 0, [{ op: "set", nodeId: "c1", path: "/capacity", value: 10 }, initial[0]!]), hasCode("NODE_NOT_FOUND"));
  const ops: GraphOp[] = [initial[4]!, initial[0]!, initial[2]!,
    { op: "set", nodeId: "c1", path: "/capacity", value: -1 },
    { op: "set", nodeId: "c1", path: "/capacity", value: 5 }];
  const result = engine.preview(draft.id, 0, ops);
  assert.deepEqual(result.changes[3]?.after, { kind: "value", value: -1 });
  assert.deepEqual(result.candidate.nodes.c1?.fields.capacity, { kind: "value", value: 5 });
...
test("clear, null and reset preserve distinct author intent with escaped field paths", () => {
  ...
  const evaluated = engine.preview(draft.id, 1, [
    ...
    { op: "remove", nodeId: "c1", path: "/capacity" },
  ...
  assert.deepEqual(evaluated.candidate.nodes.c1?.fields.capacity, { kind: "clear" });
  assert.deepEqual(evaluated.candidate.nodes.c1?.fields["a/b~c"], { kind: "value", value: "escaped" });
  assert.throws(() => engine.edit(draft.id, 1, [{ op: "set", nodeId: "c1", path: "/capacity", value: null }]), hasCode("INVALID_GRAPH"));
});
...
test("invalid op shapes, unsafe IDs, unknown fields and nested paths are rejected atomically", () => {
  ...
  const before = engine.getDraft(draft.id);
  for (const op of [
    { op: "node.add", id: "__proto__", nodeType: "document" },
    { op: "node.add", id: "new", nodeType: "missing" },
    { op: "set", nodeId: "c1", path: "/capacity", value: Infinity },
    { op: "set", nodeId: "c1", path: "/capacity", value: [] },
    { op: "set", nodeId: "c1", path: "/capacity", value: 1, surprise: true },
    { op: "set", nodeId: "c1", path: "/capacity" },
