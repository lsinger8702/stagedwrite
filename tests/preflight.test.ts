...
const initial = (fields: Record<string, string | number | null> = { capacity: 20, note: null }) => ({ roots: [{ nodeType: "project", fields }] });
async function setup(rules: ManagedRule[] = [], options: Partial<ManagedOptions> = {}) { const engine = createStagedWrite({ definitions: [definition], rules, ...options }), draft = rememberRefs(await engine.create(selector, initial()), ["c/1"]); return { engine, draft }; }
const suggestion = (d: import("../src/index.js").ManagedDraft): GraphDiagnostic => ({ code: "capacity.limit", path: `/nodes/${nodeRef(d, "c/1")}/fields/capacity`, message: "Capacity exceeds the example limit.", hint: "Choose a capacity according to user intent.", candidates: [{ value: 8, message: "An option, not a required choice.", repairOps: { patches: [{ op: "set", ref: nodeRef(d, "c/1"), scope: "canonical", path: "/capacity", value: 8 }] } }], related: [`/nodes/${nodeRef(d, "c/1")}/fields/note`] });
test("preflight: missing fields and explicit clearing are diagnosed; reset restores the original null", async () => {
    const { engine, draft } = await setup(), missing = rememberRefs(await engine.create(selector, initial({})), ["c/1"]), check = await engine.preflight(missing.id);
    assert.equal(check.status, "blocked");
    assert.deepEqual(check.diagnostics.map(d => d.path), [`/nodes/${nodeRef(missing, "c/1")}/fields/capacity`, `/nodes/${nodeRef(missing, "c/1")}/fields/note`]);
    assert.deepEqual(check.preview.nodes[nodeRef(missing, "c/1")]!.fields, { "/capacity": { kind: "undeclared" }, "/note": { kind: "undeclared" } });
    assert.equal((await engine.preflight(draft.id)).status, "passed");
    await engine.edit(draft.id, 0, { patches: [{ op: "remove", ref: nodeRef(draft, "c/1"), scope: "canonical", path: "/note" }] });
    assert.equal((await engine.preflight(draft.id)).status, "blocked");
    await engine.edit(draft.id, 1, { patches: [{ op: "reset", ref: nodeRef(draft, "c/1"), scope: "canonical", path: "/note" }] });
    assert.equal((await engine.preflight(draft.id)).status, "passed");
    assert.deepEqual((await engine.getDraft(draft.id)).fieldIntents[nodeRef(draft, "c/1")]!["/note"], { kind: "set", value: null });
    await engine.close();
});
test("preflight: concrete diagnoses are advisory and caller-selected OPs require a new check", async () => {
    const { engine, draft } = await setup([rule(d => Number(d.graph.nodes[nodeRef(d, "c/1")]!.fields.capacity) > 10 ? [suggestion(d)] : [])]), check = await engine.preflight(draft.id);
    assert.equal(check.status, "blocked");
    assert.deepEqual(await engine.getDraft(draft.id), draft);
    assert.deepEqual(check.diagnostics[0]!.candidates, suggestion(draft).candidates);
    assert.equal("resolution" in check.diagnostics[0]!, false);
    await engine.edit(draft.id, 0, { patches: [{ op: "set", ref: nodeRef(draft, "c/1"), scope: "canonical", path: "/capacity", value: 7 }] });
...
test("preflight: current check is isolated by Draft and invalidated by accepted edits", async () => {
    ...
    const other = rememberRefs(await engine.create(selector, initial()), ["c/1"]);
    await assert.rejects(engine.getCheck(other.id), /CHECK_NOT_CURRENT/);
    await engine.edit(draft.id, 0, { patches: [{ op: "set", ref: nodeRef(draft, "c/1"), scope: "canonical", path: "/capacity", value: 20 }] });
    await assert.rejects(engine.getCheck(draft.id), /CHECK_NOT_CURRENT/);
    await engine.close();
});
test("preflight: preview and rejected edits preserve the accepted check", async () => {
    ...
    await engine.preview(draft.id, 0, { patches: [{ op: "remove", ref: nodeRef(draft, "c/1"), scope: "canonical", path: "/note" }] });
    await assert.rejects(engine.edit(draft.id, 0, { patches: [] }), /no operations/);
    await assert.rejects(engine.edit(draft.id, 0, { patches: [{ op: "set", ref: nodeRef(draft, "c/1"), scope: "canonical", path: "/capacity", value: -1 }] }), /must be >=/);
...
test("preflight: throwing, async-in-sync and malformed rules cannot certify a draft", async () => {
    const bad: unknown[] = [() => { throw Error("private provider detail"); }, async () => { throw Error("async unsupported"); }, () => [{ code: "bad", path: "not-pointer" }], (d: import("../src/index.js").ManagedDraft) => [{ ...suggestion(d), severity: "fatal" }], (d: import("../src/index.js").ManagedDraft) => [{ ...suggestion(d), message: " " }], (d: import("../src/index.js").ManagedDraft) => [{ ...suggestion(d), related: ["not-pointer"] }], (d: import("../src/index.js").ManagedDraft) => [{ ...suggestion(d), candidates: [{ value: Infinity }] }], (d: import("../src/index.js").ManagedDraft) => [{ ...suggestion(d), repairs: [{ message: "bad", ops: { patches: [] } }] }], (d: import("../src/index.js").ManagedDraft) => [{ ...suggestion(d), resolution: { ops: { patches: [] } } }]];
    for (const callback of bad) {
        ...
        assert.deepEqual(await engine.getDraft(draft.id), draft);
        assert.ok(check.preview.nodes[nodeRef(draft, "c/1")]);
        await engine.close();
    }
});
test("preflight: one invalid diagnosis discards that rule's entire output", async () => {
    const { engine, draft } = await setup([rule(d => [suggestion(d), { ...suggestion(d), message: "" }])]);
...
test("preflight: frozen inputs and detached outputs cannot corrupt subsequent rules or saved state", async () => {
    let observed = 0;
    const { engine, draft } = await setup([rule(d => { d.graph.nodes[nodeRef(d, "c/1")]!.fields.capacity = 999; return []; }), { ...rule(d => { observed = Number(d.graph.nodes[nodeRef(d, "c/1")]!.fields.capacity); return [suggestion(d)]; }), id: "reader" }]);
    const check = await engine.preflight(draft.id);
    assert.equal(observed, 20);
    assert.equal(check.status, "incomplete");
    check.diagnostics.length = 0;
    check.preview.nodes[nodeRef(draft, "c/1")]!.fields["/capacity"] = { kind: "value", value: 999 };
...
test("preflight: synchronous and asynchronous callbacks receive detached managed intent snapshots", async () => {
    ...
    const inspect = (d: import("../src/index.js").ManagedDraft) => {
        calls++;
        assert.equal(d.graph.nodes[nodeRef(d, "c/1")]!.fields.note, null);
        assert.deepEqual(d.fieldIntents[nodeRef(d, "c/1")]!["/note"], { kind: "set", value: null });
        assert.deepEqual(d.initialSnapshot.graph.nodes[nodeRef(d, "c/1")]!.fields, { capacity: 20, note: null });
        assert.throws(() => { d.graph.nodes[nodeRef(d, "c/1")]!.fields.capacity = 999; }, TypeError);
        assert.throws(() => { d.initialSnapshot.graph.nodes[nodeRef(d, "c/1")]!.fields.capacity = 999; }, TypeError);
        return [];
    };
...
test("preflight: an older preview contract must be checked again before first publication", async () => {
    ...
    const engine = createStagedWrite({ definitions: [definition], ...backend, executors: [{ ...selector,
        id: "contract", version: "1", target: "mock", plan: d => [{ id: "create", payload: {}, effect: { kind: "create", nodeId: nodeRef(d, "c/1") } }],
        apply: async () => ({ kind: "applied", remoteRef: "created" }), reconcile: { unsupported: "test" }
    }] });
    try {
        const draft = rememberRefs(await engine.create(selector, initial()), ["c/1"]), check = await engine.preflight(draft.id);
