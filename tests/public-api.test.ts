...
test("package exposes one engine factory and its schema/storage helpers", async () => {
    assert.deepEqual(Object.keys(api).sort(), ["DefinitionAssemblyError", "EditInputError", "createAgentTools", "createMemoryBackend", "createSqliteBackend", "createStagedWrite", "defineDraftType", "editBatchSchema", "initialIntentSchema"].sort());
