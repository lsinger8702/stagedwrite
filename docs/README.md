...
- 当前代码：0.0.1 内存执行原型，以及已完成的 M1 定义注册与空图引擎。新引擎尚未连接编辑、预检与发布。
- 下一版目标：0.1.0 核心 MVP，见 mvp.md；这是计划，尚未完成。
- design/001 的 M1 接口已实现；其他设计与明确标注的后续能力仍属计划。
...
[复杂场景适配矩阵](design/002-complex-draft-fit.md) 区分核心已规划能力、行为不兼容和后续扩展。M1 已支持定义注册和空图；复杂字段、图 OP、基线恢复尚未实现。
