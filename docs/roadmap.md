...
**2026-09-17：[三态 OP 迁移](design/022-three-state-op-migration.md) 已按 [账本](tasks/three-state-op-migration.md) 结项。下一优先级恢复已批准的 update 派发计划；当前运行时仍未开放 update。包级输入 schema 已交付，不代表 A1 完整 Agent helpers 已完成。**
...
## TODO：存储式预检规则与规则 Schema

**R1 · 待设计，优先级待定。2026-09-16 所有者提出：preflight 规则不应只支持写进代码的注册函数，也应考虑以数据形式持久存储的规则。当前仅记 TODO，不启动实现，不改变三态 OP 迁移顺序。**

- 目标：除了代码规则，允许从存储加载声明式规则并参与同一 preflight；这里存储的是规则定义，不是检查结果缓存。
- 前置设计：明确规则 Schema（身份/版本、适用范围、条件表达、问题定位、message 与可选 hint/候选值/候选 repair OP），并确定规则的加载与执行边界。字段清单是待讨论项，不是已定接口。
- 待厘清：代码规则与存储规则如何共存；动态规则版本如何绑定检查凭据；同步/异步检查与 pending 如何沿用现有契约；支持哪些可序列化条件，哪些复杂逻辑继续使用代码。
- 保留原则：返回当前 Draft preview 和实际命中的具体诊断；候选修复不是必填项，不替 LLM 决定或执行修复；不把存储规则视为可任意执行的代码。
- 启动时先出方案与任务拆分，再确定优先级和验收，不在本轮顺带扩展规则引擎。

## TODO：评估普通 Graph 的持久化投影

**R2 · 三态公开协议与读路径收敛后再评估，不在本轮实施。** 当前 fieldIntents 是声明权威，graph.nodes[].fields 是原子保存并校验的物化投影。

- 比较继续物化与读取时派生的存储体积、读取/编辑开销、快照隔离、Artifact 摘要和证据格式成本。
- 读取时派生可能消除两份持久数据不一致这一类问题，但不会替代声明形态、Schema、身份、历史不可变性和执行事实校验。
- 不提前删除现有校验，也不改变公开普通 Graph。若结论涉及修改 000 P2 的持久化模型，必须先与项目所有者讨论，再定方案和任务；不将本 TODO 视为实施授权。
