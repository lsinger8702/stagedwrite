...
## 当前 create 契约（2026-09-14 修正）

**Draft 创建时承接本次初始工作意图。M1 的空图是历史阶段实现，不是产品定位。**

`engine.create(selector, initial)` 中 `initial: GraphInitialIntent` 包含 `nodes` 与 `edges`，节点字段使用现有三态信封；未声明字段省略，清空为 `{kind:"clear"}`，显式值为 `{kind:"value",value:...}`。至少一个节点，可以只声明部分字段。schema 不自动实例化节点或填默认值。

创建前统一校验字段 schema、节点类型与关系端点，整图成功后才首次写入，返回 version 0。结构失败不留下空容器。创建不执行业务规则或远端请求，业务冲突由 preflight 给出诊断；后续 edit 才递增版本。Draft ID、摘要、墓碑和执行记录不能由 initial 注入。

旧 `create(selector)` 暂保留以兼容已有调用与阶段测试，只会创建空图，不作为新接入示例的推荐流程。后续是否移除这一兼容入口单独处理。
