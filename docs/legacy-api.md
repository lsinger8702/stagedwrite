...
**Historical protocol only: use `createLegacyStagedWrite`. New projects use the managed lifecycle in [018](design/018-draft-lifecycle-proposal.md). These independent-Run, reset and ownership semantics do not apply to the new default API.**

# StagedWrite

**Design direction: [project principles](design/000-project-principles.md). Any conflicting principle change must be discussed with and explicitly agreed by the project owner first.**

**Graph publish creates an independent Run. Supply `{runId}` to identify one submission: resending that ID observes its Run; `resume(runId)` advances it. Different IDs mean different intents, even for the same Draft. See [the implemented contract](design/006-graph-execution.md) and [actual input/output walkthrough](examples/publish-resume-walkthrough.md).**
...
```ts
const engine = createLegacyStagedWrite({
...
exit followed by explicit recovery using a local simulated receipt ledger. See [M6 design](design/012-restart-recovery.md).
...
file with an older binary is not a rollback procedure. See [the upgrade guide](design/017-upgrade-and-trial.md).
...
See [retention policy](design/014-retention.md) and [review decisions](design/013-review-decisions.md).
...
See [the interface contract](design/015-existing-objects.md) and run `npm run demo:import`.

## Define and edit a graph

```ts
import { createLegacyStagedWrite, defineDraftType } from "stagedwrite";
...
There is no queue or background polling. See [the async rule contract](design/004-graph-preflight.md).
...
The legacy `StagedWrite` class is deprecated and retained for compatibility through the planned 0.1.0 release; use `createLegacyStagedWrite` for new integrations. It remains memory-only and does not receive graph persistence/import features.
...
See [the continuation contract](design/008-partial-continuation.md).
...
**Real-account verification is still pending.** Offline HTTP contract tests are not a substitute. The network demo is opt-in and excluded from CI. See [setup and acceptance](design/005-stripe-adapter-experiment.md) and [the updated contract](design/006-graph-execution.md).
...
- [Chinese documentation index](README.md)
- [MVP scope](mvp.md), [roadmap](roadmap.md), [architecture](architecture.md)
- [M1 definitions](design/001-registry-and-draft.md), [M2 operations](design/003-graph-operations.md), [M3 checks](design/004-graph-preflight.md)
- [Graph execution and Stripe recovery](design/006-graph-execution.md)
...
See [measured performance](benchmarks/baseline.md) and [release readiness](design/016-release-readiness.md).
