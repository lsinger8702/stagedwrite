...
| 能力 | 解决什么问题 |
|---|---|
| **执行前预检** | 检查复杂 JSON 的字段与业务约束，把可提前发现的问题挡在真实写入之前 |
| **针对性诊断** | 返回完整 preview、错误坐标和具体 message，可附候选值与修复 OP，让模型知道该改哪里 |
| **三态局部修复** | 模型只输出 set/remove/reset，由后端原子应用，减少重生成整个 body 的 token 输出与无关变更 |
| **幂等与重复发布防护** | 同一请求安全重试复用 key；已有未决 Run 时不另开 Run 重建资源 |
| **并发控制** | 版本 CAS 拒绝过时编辑，Draft 租约与存储 fencing 防止并发执行者互相覆盖状态 |
| **未知结果查证** | 超时或回执丢失进入 unknown，保留原请求证据，不能当作没生效直接重发 |
| **部分成功与断点续作** | 成功节点不重发，失败部分可修复，再 resume 同一 Run；持久后端保留恢复依据 |
| **长期 Draft 与远端更新** | 保留资源绑定，后续通过 diff/drift 更新原资源；无变化时明确 noop，不发多余请求 |
| **可接入现有 Agent** | 提供通用工具、Messages 修复循环和官方 DSH 适配；规则、请求映射、存储与锁有明确接入边界 |
...
**避免重复创建。** 首次发布原子认领执行 Run；同一 Draft 有未完成 Run 时，再次 publish 只观察已有执行，不另开一次创建。同一请求的安全重试复用原幂等键。修复改变了请求内容，必须先解决旧请求结果，再为新请求使用新的身份，避免既重复创建、又把旧错误回执当新结果。

**避免并发覆盖。** 编辑携带 expectedVersion，通过 CAS 拒绝过时修改；Draft 级租约覆盖发布、续作和编辑的临界区。存储原子校验租约 token/fence，失去执行权的旧 worker 不能继续提交状态。锁约束本系统的执行者，不会撤回已经发送的请求，也不能代替远端自己的条件写。

**把“不知道是否成功”保存下来。** 派发前先记录原请求与 key；适配器返回 applied、not_applied 或 unknown。超时、回执丢失不能被当作“没成功”。resume 使用原请求身份查证；要确认 no_effect，必须有证据表明旧请求未生效且不会再生效，仅仅“查不到”不够。

**部分成功是正常状态。** 已成功节点的回执与 Binding 逐步保存，在当前未完成 Run 内受到保护。其他节点失败后，可以根据诊断修复，再 resume 同一 Run，成功部分不重发。配置持久后端后，进程退出或重开也能根据保存的执行事实恢复；无需把整批请求重新开始。

这些机制共同减少重复副作用，但远端幂等与可靠查证仍需适配器支持，库不承诺通用 exactly-once。当前不提供自动 rollback、后台重试调度或内置的生产跨主机后端；[恢复边界](#恢复边界)与[锁和存储边界](#锁与存储边界)说明具体支持范围。
