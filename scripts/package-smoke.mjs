...
assert.deepEqual(Object.keys(api).sort(),['DefinitionAssemblyError','EditInputError','createMemoryBackend','createSqliteBackend','createStagedWrite','createAgentTools','repairDraft','repairDecisionSchema','defineDraftType','editBatchSchema','initialIntentSchema'].sort());
...
const stopped=await api.repairDraft({tools:agent,draftId:d.id,goal:"Check only",maxToolCalls:1,decide:async()=>({kind:"stop",reason:"Host review"})});assert.equal(stopped.status,"stopped");assert.ok(stopped.message&&stopped.hint);
...
`);
  ...
  writeFileSync(join(dir, 'managed.mts'), `import {createStagedWrite,createMemoryBackend,type ManagedEditResult,type ManagedDraft,type ManagedExecutor,type DraftLease,type ManagedStore,createAgentTools,repairDraft,editBatchSchema,initialIntentSchema} from 'stagedwrite';\nconst e=createStagedWrite({definitions:[],...createMemoryBackend()});\nfunction check(d:ManagedDraft,x:ManagedExecutor,l:DraftLease,s:ManagedStore){const name=d.graph.nodes.one?.fields.name;return [name,x.plan(d),l.fence,s.namespace];}\nconst edit: Promise<ManagedEditResult> = e.edit("draft", 0, {patches:[]});\nconst agent = createAgentTools({engine:e,definition:{id:"d",version:"1",nodeTypes:{t:{valueSchema:{type:"object",properties:{},additionalProperties:false}}},relationTypes:{}},authorize:()=>false}); const ctx=agent.invoke("stagedwrite_context",{draftId:"d"}); void repairDraft({tools:agent,draftId:"d",goal:"Check",decide:async()=>({kind:"stop",reason:"Review"})}); void ctx; void edit;void e;void check;void editBatchSchema;void initialIntentSchema;\n`);
