...
const results = [];
for (const nodes of (full ? [100, 1000, 10000] : [100, 1000])) for (const ruleCount of (full ? [0, 10, 100] : [0, 10])) for (const suggestions of (ruleCount ? [false, true] : [false])) {
  const rules = Array.from({ length: ruleCount }, (_, i) => ({ ...selector, id: `rule-${i}`, version: '1', check: () => suggestions ? [{ code: 'value', path: '/nodes/n0/fields/value', message: 'Set a value', resolution: { kind: 'ops', ops: [{ op: 'set', nodeId: 'n0', path: '/value', value: 1 }] } }] : [] }));
  const engine = createStagedWrite({ definitions: [definition], rules }); const draft = engine.create(selector);
  ...
  times.sort((a, b) => a - b);
  results.push({ nodes, rules: ruleCount, suggestionsPerRule: Number(suggestions), samples: 5, medianMs: +times[2].toFixed(2), maxMs: +times[4].toFixed(2), heapUsedMiB: +(process.memoryUsage().heapUsed / 1048576).toFixed(2) }); engine.close();
}
const storage = [];
for (const generations of [1, 10, 100]) {
  const dir = mkdtempSync(join(tmpdir(), 'sw-capacity-')), path = join(dir, 'data.sqlite');
  try {
    const e = createStagedWrite({ definitions: [definition], mode: 'executable', storage: { kind: 'sqlite', path }, executors: [{ ...selector, id: 'bench', version: '1', target: 'mock', plan: () => [{ id: 'one', payload: {} }], apply: async () => ({ kind: 'not_applied', reason: 'fixture refusal' }), reconcile: { unsupported: 'fixture' } }] });
