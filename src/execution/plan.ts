...
/** Shared validation for both public entry points and the runtime boundary. */
export function validatePlan(output: unknown): Step[] {
  ...
  for (const step of plan) {
    ...
    if (!Array.isArray(deps) || new Set(deps).size !== deps.length || deps.some(d => typeof d !== "string" || !ids.has(d))) throw new Error("INVALID_PLAN");
    if (Object.hasOwn(step, "dependsOn") && !Array.isArray(step.dependsOn)) throw new Error("INVALID_PLAN");
