import { selector, catalogRefs } from './schema.mjs';
...
export function createCatalogExecutor({ request, experiment, target, loseAnnualReceipt = false }) {
  return {
    ...
    plan(draft) {
      const refs = catalogRefs(draft);
      return ['product', 'monthly', 'annual'].map(id => {
        const node = draft.graph.nodes[refs[id]];
        return { id, effect: { kind: 'create', nodeId: node.id }, payload: { ...node.fields },
          ...(id !== 'product' ? { dependsOn: ['product'], inputRefs: { product: 'product' } } : {}) };
      });
    },
    async apply(step, key, { signal }) {
      ...
      if (step.id !== 'product' && result.status === 400 && error?.type === 'invalid_request_error' && error.param === 'currency') {
        ...
        return { kind: 'not_applied', reason: message, code: 'STRIPE_CURRENCY_VALIDATION', message,
          diagnostics: [{ code: 'STRIPE_CURRENCY_VALIDATION', path: `/nodes/${step.effect.nodeId}/fields/currency`, message,
            candidates: [{ value: 'hkd', label: 'HKD', repairOps: { patches: [{ op: 'set', ref: step.effect.nodeId, scope: "canonical", path: '/currency', value: 'hkd' }] } }] }] };
      }
