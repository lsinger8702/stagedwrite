...
const executor: GraphExecutor = { ...selector, id: "example.dependencies", version: "1", target: "mock:local",
  plan: draft => {
    ...
    return [
      ...
      { id: edge.to, payload: { name: name(edge.to) }, dependsOn: [edge.from], inputRefs: { campaignId: edge.from } }
    ];
  },
  apply: async (step: Step) => {
    if (step.id === "campaign") {
      if (rejectCampaign) return { kind: "not_applied", reason: "Synthetic name refusal" };
...
engine.edit(revised.id, 0, [{ op: "set", nodeId: "campaign", path: "/name", value: "Accepted name" }]);
rejectCampaign = false;
